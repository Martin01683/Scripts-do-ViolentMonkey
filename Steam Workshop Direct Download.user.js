...
(function() {
    ...
    const TemplateEngine = {
         ...
         * de colar esses blocos manualmente na string. Foi exatamente o esquecimento
         * desse "colar manual" que causou o bug de "Mirrors verificados" sumindo nos
         * estados de Pedir Mod / Mod Indisponível.
         *
         * @param {object} config - Objeto contendo o Schema de renderização
         */
    ...
    function getSteamDateAsync(modId) {
        return new Promise(resolve => {
            // AUTOVALIDAÇÃO (CORREÇÃO DE BUG): diferente do cache dos mirrors
            // (fetchMirrorAsync, que sempre confere "exp > now" sozinho a cada chamada),
            // este cache em memória (steamDateCache) era invalidado SOMENTE pelo heartbeat
            // externo (setInterval de 1s, mais abaixo no arquivo) — e esse heartbeat só
            // roda quando a aba está visível, o usuário não está ocioso e o widget ainda
            // consta em "activeWidgets". Se a expiração acontecesse exatamente quando
            // alguma dessas condições não era satisfeita (aba em segundo plano, GC/engasgo
            // do navegador, etc.), o heartbeat perdia a janela de invalidar o cache e
            // "steamDateCache[modId]" ficava com o valor antigo PARA SEMPRE — daí o tooltip
            // mostrar o cache da Steam travado em "0s", sem se recuperar nem com o usuário
            // movendo o mouse ou rolando a página depois. Validando a expiração aqui
            // também, a cada chamada, o cache se autocorrige independentemente do
            // heartbeat ter ou não rodado no momento exato da expiração.
    ...
    async function renderWidget(container, modId, isCard) {
        ...
        if (!mirrorResult || mirrorResult.notFound) {
            ...
            // BUG CORRIGIDO: este branch (mod não encontrado) nunca preenchia
            // "activeMirrorIds", então o heartbeat não tinha como saber quais mirrors foram
            // consultados aqui — o cache desses mirrors podia expirar e o widget nunca
            // seria atualizado automaticamente enquanto permanecesse nesse estado.
        ...
        // quando o mirror não possui data, ou quando a Steam não pôde ser consultada (regressão corrigida).
